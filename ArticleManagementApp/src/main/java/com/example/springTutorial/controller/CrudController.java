...
public interface CrudController <ENTITY_TYPE> {
    ...
    @GetMapping("/{id}")
    default ResponseEntity<ENTITY_TYPE> read(@PathVariable Long id){
    ...
    @PutMapping("/{id}")
     default ResponseEntity<ENTITY_TYPE> update(@PathVariable Long id,
                                                @RequestBody ENTITY_TYPE entity) {
